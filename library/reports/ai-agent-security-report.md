...
1. [一、攻击面与威胁分类](#一-攻击面与威胁分类-security-threats)
2. [二、防御方法与最佳实践](#二-防御方法与最佳实践-defense-mitigations)
3. [三、安全标准、框架与合规](#三-安全标准-框架与合规-standards-frameworks)
4. [四、真实事件案例与生态格局](#四-真实事件案例与生态格局-incidents-ecosystem)
5. [五、结论与行动建议](#五-结论与行动建议)
6. [六、信息来源核验表](#六-信息来源核验表-fact-check-appendix)
...
- 若需经济量化:见 [05 章 5.2 经济损失与 ROI 模型](#五-结论与行动建议)(依赖本表 L/I 值)
- 若需检测落地:见 [02 章 2.9 检测与响应资产包](#二-防御方法与最佳实践-defense-mitigations)(Sigma 规则对应本表高/极危向量)
- 若需合规映射:见 [03 章 3.7 合规落地映射](#三-安全标准-框架与合规-standards-frameworks)
...
> 下表汇总 Agent 安全相关的主要标准/框架及其最新版本。各框架详细内容见 3.2-3.6 节；核验明细见 [06-来源核验表](#六-信息来源核验表-fact-check-appendix)。
...
> 下表汇总可用于 agent 安全评估的公开基准。各基准的评测方法详见 3.9 节与论文原文；核验明细见 [06-来源核验表](#六-信息来源核验表-fact-check-appendix)。
...
- **Art.9 风险管理**：将工具滥用视为"可合理预见的误用"；风险登记表按 agent 能力/自主级别建账（对应本报告 [风险登记表（01 章 1.7）](#一-攻击面与威胁分类-security-threats)）
...
- 若需把合规义务量化为控制成本:见 [经济损失与 ROI 模型（05 章 5.2）](#五-结论与行动建议)
- 若需合规证据的技术支撑:见 [检测与响应资产包（02 章 2.9）](#二-防御方法与最佳实践-defense-mitigations)（审计日志章节）
...
**现状**：42001 的 38 项 Annex A 控制为组织级,无工具使用策略/agent 身份/委派/记忆（见 [合规落地映射（3.7 节）](#3-7-合规落地映射-义务-控制) 3 节）。
...
- 若需把"标准滞后"量化成企业风险:见 [风险登记表（01 章 1.7）](#一-攻击面与威胁分类-security-threats)
...
- 若需落地已成熟的部分:见 [MCP 安全实操包（02 章 2.10）](#二-防御方法与最佳实践-defense-mitigations)（dual-prompt 等已是实践而非研究）
...
- 若需风险视角:见 [风险登记表（01 章 1.7）](#一-攻击面与威胁分类-security-threats)
- 若需厂商/开源工具细节:见 [02-防御方法与最佳实践](#二-防御方法与最佳实践-defense-mitigations) 2.8
- 若需合规市场驱动:见 [合规落地映射（03 章 3.7）](#三-安全标准-框架与合规-standards-frameworks)

## 4.7 厂商声明审计与公共政策

> 本节提供**信息可信度审计**与**政策讨论**两个视角。审计表严格复用 [06-来源核验表](#六-信息来源核验表-fact-check-appendix) 结果,**不新增未经核验的裁决**；政策 Q&A 区分"现行法律事实"与"政策建议"。

### 执行摘要（BLUF）

**如果只记住一件事**：**AI 安全行业的信息生态里,夸大是常态,不是例外**——本报告 79 条声明核验发现 13 条需修正、1 条删除（见 [06-来源核验表](#六-信息来源核验表-fact-check-appendix)）;其中不乏"把未成功的攻击说成绕过""把未证实数据当事实引用""把模拟实验当真实事件"的典型案例。**读者必须养成三个习惯:查一手来源、区分演示与实战、警惕厂商口径数字。**
...
> 本表复用 [06-来源核验表](#六-信息来源核验表-fact-check-appendix) 结果，按"声明→核验→裁决"格式给出每条声明的审计结论。
...
- **现行事实**：EU Art.50(1) 要求告知"与 AI 交互"；中国标识办法要求 AI 生成内容显式标识（[合规落地映射（03 章 3.7）](#三-安全标准-框架与合规-standards-frameworks)）。但均未要求披露**自主级别/工具权限**。
...
- **现行事实**：EU AI Act 将招聘 AI 列为 Annex III 高风险（[合规落地映射（03 章 3.7）](#三-安全标准-框架与合规-standards-frameworks) 4 节）；但无强制第三方安全审计条款。
...
- 若需完整核验明细:见 [06-来源核验表](#六-信息来源核验表-fact-check-appendix)
- 若需事件背景:见本章 4.1-4.5
- 若需合规事实基础:见 [合规落地映射（03 章 3.7）](#三-安全标准-框架与合规-standards-frameworks)
...
> 这些是**已证实的事实**,引用时无需加估算声明。核验明细见 [06-来源核验表](#六-信息来源核验表-fact-check-appendix)。
...
**示例**（基于 [风险登记表（01 章 1.7）](#一-攻击面与威胁分类-security-threats) 的 Top 向量；注：两行 ALE 视为**不重叠的独立场景**——Trifecta 外泄为数据泄露损失，提示注入直接损失为操作/恢复损失，合计时忽略重叠部分，属保守估算上限）：
...
- 若需确定哪些向量值得投资:见 [风险登记表（01 章 1.7）](#一-攻击面与威胁分类-security-threats)
- 若需落地控制:见 [MCP 安全实操包（02 章 2.10）](#二-防御方法与最佳实践-defense-mitigations) 与 [检测与响应资产包（02 章 2.9）](#二-防御方法与最佳实践-defense-mitigations)
